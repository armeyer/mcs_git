...
order. Some answers may be used multiple times or not at all.  Enter the number of the correct solution in the box below each
question. No explanations are required.
...
\ppart How many total functions\footnote{Recall that \emph{total function} means \emph{exactly one arrow out}.} are there from a set of size $m$ to a
set of size $n$?
...
\ppart How many total injective\footnote{Recall that \emph{injective} means \emph{at most one arrow in}.} functions are there from set of size
